...
import com.zhou.pojo.TbUserExample.Criteria;
...
public class RegisterServiceImpl implements RegisterService {
	...
	@Override
	public ShopResult checkData(String param, int type) {
		//根据不同的type生成不同的查询条件
	...
	public ShopResult register(TbUser user) {
		...
		if (StringUtils.isBlank(user.getUsername()) || StringUtils.isBlank(user.getPassword()) 
			...
			return ShopResult.build(400, "用户数据不完整，注册失败");
		}
		//1：用户名 2：手机号 3：邮箱
		ShopResult result = checkData(user.getUsername(), 1);
		if (!(boolean) result.getData()) {
			return ShopResult.build(400, "此用户名已经被占用");
		}
		result = checkData(user.getPhone(), 2);
		if (!(boolean)result.getData()) {
			return ShopResult.build(400, "手机号已经被占用");
		}
		//补全pojo的属性
